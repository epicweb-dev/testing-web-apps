...
1. `page.goto` is like the user entering a URL in the address bar and hitting
   "enter". Because often we want to test our app, we can skip the full URL and
   just provide a pathname by configuring a
...
2. `page.getByRole` is called a [locator](https://playwright.dev/docs/locators).
   ...
   technologies. By querying using roles, you're also testing a **part** of your
   application's accessibility to people using these technologies.
